...
class MatchUILaya {
    ...
    _createUI() {
        console.log('MatchUI_createUI()');
        // 设计分辨率：指设计图纸分辨率
        const _designWidth = 1125;
        const _designHeight = 2001;
        this._scale =
            Laya.stage.width / _designWidth < Laya.stage.height / _designHeight
                ? Laya.stage.width / _designWidth
                : Laya.stage.height / _designHeight;
    ...
    _updateHeads(_playerNum, _playerInfoList, _ifDrawHeads = false) {
        ...
        for (let _i = 0; _i < _playerNum; _i++) {
            ...
            this._headbgImg.addChild(_headGroup);
            // 绘制头像
            ...
            if ((_i < _playerInfoList.length && _ifDrawHeads) || _i === 0) {
                ...
                localImage.onload = () => {
                    ...
                    const t2d = new Laya.Texture2D(localImage.width / scale, localImage.height / scale, 
                    // (Laya as any).TextureFormat.R8G8B8A8
            ...
            if (_i < _playerInfoList.length && _playerInfoList[_i].isAnchor) {
                ...
                _redcircleimg.width = _redcircleimg.height =
                    _playerNum < 3 ? 204 : 162;
            ...
            if (_namelbl.width > _headGroup.width * 1.2) {
                ...
                while (true) {
                    ...
                    _namelbl.autoSize = true;
                    if (i === str.length ||
                        _namelbl.width > _headimg.width * 1.2) {
                        break;
                    }
                }
            }
    ...
    show(_playerNum, _userInfo, _debug = false) {
        ...
        console.log('MatchUI_show()_End');
    }
    matchSuccess(_playerList) {
        console.log('MatchUI_matchSuccess()');
        this._closePromise = new Promise(resolve => {
            this._closeResolve = resolve;
        });
        const _playerInfoList = [];
        // 把我放在第一位
        ...
        Laya.Tween.to(this._wait, { scaleX: 0.01, scaleY: 0.01 }, 500, Laya.Ease.backOut, Laya.Handler.create(this, () => {
            this._success.visible = false;
            this._success.scaleX = 0.01;
            this._success.scaleY = 0.01;
            this._success.visible = true;
            Laya.Tween.to(this._success, { scaleX: this._scale, scaleY: this._scale }, 500, Laya.Ease.backOut, Laya.Handler.create(this, () => {
                this._headbgImg.removeChildren();
...
class DebugUI {
    ...
    _createUI() {
        ...
        console.log('Debug_createUI()_End');
    }
    show() {
        console.log('Debug_show()');
        this._debugbtn.visible = true;
        console.log('Debug_show()_End');
    }
    // @ts-ignore
    log(...arg) {
        if (this._logString) {
        ...
        for (let i = 0; i < arg.length; i++) {
            if (Array.isArray(arg[i])) {
                ...
                for (let index = 0; index < param.length; index++) {
                    this._logString += typeof param[index] === 'string' ? param[index] : JSON.stringify(param[index], DebugUI.replace);
                    DebugUI._seen = [];
                    this._logString += ' ';
                }
            }
            else {
                this._logString += typeof arg[i] === 'string' ? arg[i] : JSON.stringify(arg[i], DebugUI.replace);
                DebugUI._seen = [];
                this._logString += ' ';
            }
        }
    ...
    static replace(key, value) {
        // 避免假卡死，对象检索的深度深于一定深度就跳出显示
        if (DebugUI._seen.length > 30) {
            return;
        }
...
class MicrophoneUILaya {
    ...
    _createUI() {
        console.log('MicrophoneUILaya_createUI()');
        // 设计分辨率：指设计图纸分辨率
        const _designWidth = 1125;
        const _designHeight = 2001;
        const _scale = Laya.stage.width / _designWidth < Laya.stage.height / _designHeight
            ? Laya.stage.width / _designWidth
            : Laya.stage.height / _designHeight;
        this._dialog = new Laya.Scene();
        this._mySprite = new Laya.Image();
        this._otherSprite = new Laya.Image();
        // this._dialog.isPopupCenter = false;
        this._dialog.width = 249 * _scale;
        this._dialog.height = 96 * _scale;
        this._dialog.y = 51 * _scale;
        this._dialog.x = 22; // Laya.stage.width - this._dialog.width;
        this._mySprite.width = this._mySprite.height = this._otherSprite.width = this._otherSprite.height =
            96 * _scale;
        ...
        console.log('MicrophoneUILaya_createUI()_End');
    }
    show(config) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            console.log('MicrophoneUILaya_showUI()');
            try {
                yield pvp_microphone.PVPMicrophone.init({
            ...
            try {
                yield pvp_microphone.PVPMicrophone.startChat();
            }
            catch (e) {
